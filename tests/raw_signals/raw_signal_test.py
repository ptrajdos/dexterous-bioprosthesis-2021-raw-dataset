...
class RawSignalTest(unittest.TestCase):
    ...
    def test_rawsignal(self):
        ...
        obj2 = RawSignal(signal=sig,timestamp=333, object_class=0,channel_names=["X{}".format(i) for i in range(C)])
    ...
    def test_equaity(self):
        ...
        obj2 = RawSignal(signal=sig,timestamp=10,object_class=0,channel_names=["X{}".format(i) for i in range(C)])
        self.assertTrue( obj != obj2, "Object should not have been equal")        
    ...
    def test_getitem(self):
        ...
        S = 20
        s_obj = obj[:S]
        self.assertTrue( s_obj.signal.shape == (S,obj.signal.shape[1]), "One index. Shapes after selection" )
        self.assertTrue( s_obj.channel_names == obj.channel_names, "One index. Channel names")

        # one element tuple
        s_obj = obj[:S,]
        self.assertTrue( s_obj.signal.shape == (S,obj.signal.shape[1]), "One elem tuple. Shapes after selection" )
        self.assertTrue( s_obj.channel_names == obj.channel_names, "One slem tuple. Channel names")

        # int selection
        s_obj = obj[[S]]
        self.assertTrue( s_obj.signal.shape == (1,obj.signal.shape[1]), "Int. Shapes after selection" )
        ...
        SC=3
        s_obj = obj[:S, :SC]
        self.assertTrue( s_obj.signal.shape == (S,SC), "Two indices, slice. Shapes after selection" )
        self.assertTrue( s_obj.channel_names == obj.channel_names[:SC], "Two indices, slice. Colum names" )

        # Collection
        SC=[1,2,3]
        s_obj = obj[:S,SC]
        self.assertTrue( s_obj.signal.shape == (S,len(SC)), "Two indices, collection. Shapes after selection" )
        self.assertTrue( s_obj.channel_names == [obj.channel_names[i] for i in SC], "Two indices, collection. Colum names" )

    
        # Boolean slicing
        bool_sel = [bool(1) for  i in range(C)]
        obj_cp = obj[:,bool_sel]
        self.assertTrue( obj == obj_cp, "Whole slicing via booleans. Objects should have been equal")
