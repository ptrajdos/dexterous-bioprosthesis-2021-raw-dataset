...
from dexterous_bioprosthesis_2021_raw_datasets.raw_signals_filters.raw_signals_filter_row_slice import RawSignalsFilterRowSlice
from tests.raw_signals_filters.raw_signals_filter_test import RawSignalsFilterTest


class RawSignalsFilterRowSliceBaseTest(RawSignalsFilterTest):
    """Inherited base filter tests."""
    __test__ = True

    def get_filters(self):
        return [RawSignalsFilterRowSlice(row_slice=slice(0, 5))]


class RawSignalsFilterRowSliceTest(unittest.TestCase):
    ...
    def test_select_slice(self):
        ...
        filt = RawSignalsFilterRowSlice(row_slice=slice(5, 15))
    ...
    def test_slice_from_start(self):
        ...
        filt = RawSignalsFilterRowSlice(row_slice=slice(None, 5))
        result = filt.fit_transform(signals)
        for sig in result:
    ...
    def test_slice_to_end(self):
        signals = self._make_signals(n=2, rows=20, cols=4)
        filt = RawSignalsFilterRowSlice(row_slice=slice(15, None))
        result = filt.fit_transform(signals)
        for sig in result:
            self.assertEqual(len(sig), 5)
    ...
    def test_slice_with_step(self):
        signals = self._make_signals(n=2, rows=20, cols=4)
        filt = RawSignalsFilterRowSlice(row_slice=slice(0, 20, 2))
        result = filt.fit_transform(signals)
        for sig in result:
            self.assertEqual(len(sig), 10)
    ...
    def test_preserves_metadata(self):
        signals = self._make_signals(n=2, rows=20, cols=4)
        filt = RawSignalsFilterRowSlice(row_slice=slice(0, 10))
        result = filt.fit_transform(signals)
    ...
    def test_preserves_channel_names(self):
        ...
        filt = RawSignalsFilterRowSlice(row_slice=slice(0, 5))
        result = filt.fit_transform(signals)
    ...
    def test_preserves_values(self):
        ...
        filt = RawSignalsFilterRowSlice(row_slice=slice(5, 10))
        result = filt.fit_transform(signals)
    ...
    def test_negative_slice(self):
        ...
        filt = RawSignalsFilterRowSlice(row_slice=slice(-5, None))
        result = filt.fit_transform(signals)
