...
from dexterous_bioprosthesis_2021_raw_datasets.set_creators.set_creator_window_delta_diff import (
    SetCreatorWindowDeltaDiff,
)
...
class SetCreatorWindowDeltaDiffTest(SetCreatorTest):

    __test__ = True

    def get_creators(self):
        extractors = {
            "default": SetCreatorWindowDeltaDiff(extractors=[
                NpSignalExtractorMav(),
                NpSignalExtractorSsc(),
            ]),
            "w_0.5;o_0.5": SetCreatorWindowDeltaDiff(
                extractors=[
                    NpSignalExtractorMav(),
                    NpSignalExtractorSsc(),
                ],
    ...
    def test_output_shape_matches_expected_features(self):
        """X columns should equal n_extractors * n_deltas * n_channels."""
        creator = SetCreatorWindowDeltaDiff(
        ...
        X, y, t = creator.fit_transform(raw_set)
        # 3 windows → 2 deltas; 2 extractors * 2 deltas * 3 channels = 12
    ...
    def test_n_windows_and_n_deltas_stored_after_fit(self):
        """After fit, n_windows and n_deltas should be set correctly."""
        creator = SetCreatorWindowDeltaDiff(
            extractors=[NpSignalExtractorMav()],
            window_length=0.5, overlap=0.5,
        )
        ...
        self.assertEqual(creator.n_deltas, 2)

    def test_single_window_no_overlap(self):
        """When window covers entire signal, should produce 1 window and 0 deltas."""
        creator = SetCreatorWindowDeltaDiff(
            extractors=[NpSignalExtractorMav()],
        ...
        X, y, t = creator.fit_transform(raw_set)
        # effective_window = round(0.99*20) = 20, covers entire signal → 1 window, 0 deltas
        self.assertEqual(creator.n_windows, 1)
        self.assertEqual(creator.n_deltas, 0)
        # 0 deltas → 0 features
    ...
    def test_single_extractor(self):
        ...
        X, y, t = creator.fit_transform(raw_set)
        # 1 extractor * 2 deltas * 3 channels = 6
    ...
    def test_three_extractors(self):
        ...
        X, y, t = creator.fit_transform(raw_set)
        # 3 extractors * 2 deltas * 3 channels = 18
    ...
    def test_single_channel(self):
        ...
        X, y, t = creator.fit_transform(raw_set)
        # 2 extractors * 2 deltas * 1 channel = 4
    ...
    def test_many_channels(self):
        ...
        X, y, t = creator.fit_transform(raw_set)
        # 1 extractor * 2 deltas * 8 channels = 16
    ...
    def test_channel_attribs_indices_cover_all_features(self):
        ...
        # 2 extractors * 2 deltas * 3 channels = 12
    ...
    def test_fit_then_transform_same_as_fit_transform(self):
        ...
        creator1 = SetCreatorWindowDeltaDiff(
            extractors=[NpSignalExtractorMav(), NpSignalExtractorSsc()],
            window_length=0.5, overlap=0.5,
        )
        creator2 = SetCreatorWindowDeltaDiff(
            extractors=[NpSignalExtractorMav(), NpSignalExtractorSsc()],
            window_length=0.5, overlap=0.5,
        )
        raw_set = self.generate_sample_data(
    ...
    def test_transform_different_data_same_signal_length(self):
        ...
        self.assertEqual(X.shape[1], 6)  # 1 * 2 deltas * 3 channels

    # --- Constant signal tests ---

    def test_constant_signal_produces_zero_features(self):
        """Constant signal should produce zero delta features (all windows identical)."""
        creator = SetCreatorWindowDeltaDiff(
            extractors=[NpSignalExtractorMav()],
            window_length=0.5, overlap=0.5,
        )
        ...
        X, y, t = creator.fit_transform(raw_set)
        # MAV of constant signal is the same in every window → deltas are 0
        np.testing.assert_array_almost_equal(X, np.zeros_like(X))
