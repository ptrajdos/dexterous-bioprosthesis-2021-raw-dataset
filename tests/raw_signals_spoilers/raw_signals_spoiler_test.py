    ...
    def test_snrs(self):
        if self.is_test_snr():
            ...
            for snr in snrs:
                ...
                self.assertEqual(len(data), len(t_data), "Number of objects is different")
                self.assertEqual(data[0].to_numpy().shape, t_data[0].to_numpy().shape, "Different shapes")
                ...
                for w_signal, o_signal in zip(t_data, data):
                    ...
                    self.assertTrue(np.allclose(c_snrs, d_snrs,rtol=1e-3, atol=1e-3), "Too far from desired SNRs")
