...
class RawSignalsSpoilerInterfaceTest(unittest.TestCase):
    ...
    def _check_for_invalid_values(self, raw_signals: RawSignals):
        ...
        for signal in raw_signals:
            self.assertFalse(
                np.any(np.isnan(signal.to_numpy())), "NaNs in transformed data"
            )
            self.assertFalse(
                np.any(np.isinf(signal.to_numpy())), "Infs in transformed data"
            )

    def _check_for_dtype(self, raw_signals: RawSignals, dtype):

        for signal in raw_signals:
            self.assertEqual(
                signal.to_numpy().dtype,
                dtype,
                f"Expected dtype {dtype}, got {signal.to_numpy().dtype}",
            )
    ...
    def test_fit_then_transform(self):
        ...
        for N, C, R in rcs:
            data = self.generate_sample_data(
                signal_number=N, column_number=C, samples_number=R
            )
            for spoiler in spoilers:
                with self.subTest(spoiler=spoiler, N=N, C=C, R=R):
                    ...
                    self.assertIsInstance(
                        t_data,
                        RawSignals,
                        "Transformed data is not an instance of RawSignals",
                    )
    ...
    def test_fit_transform(self):
        ...
        for dtype in dtypes:
            data = self.generate_sample_data(dtype=dtype)
            for spoiler in spoilers:
                ...
                self._check_for_invalid_values(t_data)
                self._check_for_dtype(t_data, dtype)
