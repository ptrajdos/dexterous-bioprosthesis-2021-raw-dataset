...
class RawSignalsSpoilerSinesHarmonics(RawSignalsSpoiler):
    ...
    def __init__(self, channels_spoiled_frac=0.1, snr=1, frequency=50, freq_deviation=2, n_harmonics=2,
                 harmonic_weights=[1,1]) -> None:
    ...
    def fit(self,raw_signals:RawSignals):
        ...
        if self.n_harmonics != len(self.harmonic_weights):
            raise ValueError("The number of harmonics is inconsistent with harmonic weights.")
    ...
    def transform(self, raw_signals:RawSignals):
        ...
        for signal in copied_signals:
            ...
            for harm_id, harmonic_weight in enumerate( self.harmonic_weights ):
                harm_freq = used_frequency* (harm_id + 2)
                noise_sig_np_col += harmonic_weight * np.reshape(np.sin( 2* np.pi * harm_freq * x + phase_shift), (n_samples,-1))
