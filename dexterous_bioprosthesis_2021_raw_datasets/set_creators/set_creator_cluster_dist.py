...
from dexterous_bioprosthesis_2021_raw_datasets.set_creators.set_creator_mds import (
    no_flatten,
)
...
class SetCreatorClusterDist(SetCreator):
    ...
    def _fit(self, raw_signals: RawSignals):
        ...
        is_channel_attribute_mapping = (
            True
            if initial_distance_matrix.shape[0] == distance_matrix.shape[0]
            else False
        )
        ...
        for channel_id in range(n_channels):
            ...
            for k in range(2, max_clusters):
                ...
                sil_val = silhouette_score(
                    channel_dist_matrix, labels=clusters, metric="precomputed"
                )
                silh.append((k, sil_val, clusters))

            best_k, best_silh, best_cluster_assig = sorted(
                silh, key=lambda x: x[1], reverse=True
            )[0]
            samples_silh = silhouette_samples(
                channel_dist_matrix, labels=best_cluster_assig, metric="precomputed"
            )
        ...
        self.reference_samples = RawSignals(
            raw_signals[best_samples_ids], sample_rate=raw_signals.sample_rate
        )
        ...
        if is_channel_attribute_mapping:
            ...
            for _ in range(n_channels):
                self.channel_selected_attribs.append(
                    offset + np.arange(n_best_sample_ids)
                )
        ...
        repr = np.concatenate(
            self.flatten_function(
                self.distance_calculator.calculate_distance_matrix_set_2_set(
                    raw_signals_1=raw_signals, raw_signals_2=self.reference_samples
                ),
                keepdims=True,
            ),
            axis=1,
