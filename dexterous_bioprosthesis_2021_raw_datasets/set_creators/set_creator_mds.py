...
from dexterous_bioprosthesis_2021_raw_datasets.distance_matrix_calculators.distance_matrix_calculator_dtw_fast import (
    DistanceMatrixCalculatorDTWFast,
)
...
class SetCreatorMDS(SetCreator):
    ...
    def __init__(
        self,
        dist_matrix_calc: DistanceMatrixCalculator = DistanceMatrixCalculatorDTWFast(),
        mds_options: dict = {},
        ga_options: dict = default_ga_options,
        tqdm_disable=True,
        n_jobs=None,
        flatten_function=no_flatten,
        n_attr: int = 2,
    ...
    def _calculate_mds(self, distance_matrix):
        ...
        for n_slice in range(n_slices):
            ...
            for n_at in tqdm(
                n_attr_to_check,
                desc="Finding best number of attributes",
                total=len(n_attr_to_check),
                leave=False,
            ...
            if n_atts_final is None:
                n_atts_final = self.n_attr

            mds_obj = MDS(
                dissimilarity="precomputed",
                n_jobs=self.n_jobs,
                n_components=n_atts_final,
                normalized_stress=False,
                **self.mds_options
            )
    ...
    def create_set_from_distance_matrix(self, raw_signals, distance_matrix):
        ...
        extracted_objs_timestamps = np.asanyarray(
            [rs.get_timestamp() for rs in raw_signals]
        )
    ...
    def fit(self, raw_signals: RawSignals, y=None):
        ...
        distance_matrix = self.flatten_function(
            distance_matrix_channels, axis=0, keepdims=True
        )

        self.dataset = self.create_set_from_distance_matrix(
            raw_signals, distance_matrix
        )
    ...
    def _distance_raw_signal_training_data(self, raw_signal: RawSignal):
        ...
        for t_sig_id, training_signal in enumerate(self.raw_signals_set):
            dist = self.flatten_function(
                self.distance_matrix_calc.raw_signal_dist(training_signal, raw_signal),
                axis=0,
                keepdims=True,
            )
    ...
    def _calculate_distances_to_training_data(self, raw_signals: RawSignals):
        ...
        signal_distances_list = ProgressParallel(
            ...
            desc="Distances from training to new",
        )(
            delayed(self._distance_raw_signal_training_data)(raw_signal)
        ...
        for n_slice in range(n_slices):
            ...
            def fitness_function(solution, solution_idx):
                ...
                for i in range(len(self.raw_signals_set)):
                    val = np.square(
                        np.sqrt(
                            np.sum(np.square(solution - self.dataset[0][i, start:stop]))
                        )
                        - new_to_train_distances[n_idx, i, n_slice]
                    )
                    s_sum += val
    ...
    def initialize_population(self, start, stop):
        ...
        new_solutions_number = train_solutions.shape[0] * self.ga_options.pop(
            "sol_per_pop_mult", 1
        )
        ...
        for attr_idx in range(new_solutions.shape[1]):
            new_solutions[:, attr_idx] = np.random.uniform(
                low=min_vals[attr_idx],
                high=max_vals[attr_idx],
                size=(new_solutions_number,),
            )
    ...
    def transform(self, raw_signals: RawSignals):
        ...
        new_X = ProgressParallel(
            ...
            use_tqdm=not self.tqdm_disable,
            total=len(raw_signals),
            desc="Fitting new points",
        )(
            delayed(self._new_X_vec)(new_to_train_distances, i)
            for i in range(len(raw_signals))
        )

        tr_dataset = np.asanyarray(new_X).astype(
