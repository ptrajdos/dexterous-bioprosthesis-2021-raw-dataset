...
class DistanceMatrixCalculatorDTWFast2(DistanceMatrixCalculator):
    ...
    def calculate_distance_matrix(self, raw_signals: RawSignals):
        ...
        pair_dist_list_pre = ProgressParallel(
            n_jobs=self.n_jobs, use_tqdm=not self.tqdm_disable, total=total_iter_number
        )(
            delayed(self._compute_raw_signals_dist)(raw_signals, i)
    ...
    def raw_signal_dist_2_set(self, raw_signal: RawSignal, raw_signals: RawSignals):
        ...
        dist_list = ProgressParallel(
            n_jobs=self.n_jobs, use_tqdm=not self.tqdm_disable, total=total_dist_number
        )(
            delayed(self._compute_raw_signal_2_set_dist)(raw_signals, raw_signal, i)
            for i in range(num_signals)
        )
    ...
    def calculate_distance_matrix_set_2_set(
        ...
        pair_dist_list = ProgressParallel(
            n_jobs=self.n_jobs, use_tqdm=not self.tqdm_disable, total=total_pair_number
        )(
            delayed(self._compute_raw_signals_dist_2_set)(
                raw_signals_1, raw_signals_2, i, j
            )
            for i, j in product(range(num_signals_1), range(num_signals_2))
        )
