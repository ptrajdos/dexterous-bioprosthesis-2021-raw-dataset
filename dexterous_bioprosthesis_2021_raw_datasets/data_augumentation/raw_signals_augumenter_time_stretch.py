...
class RawSignalsAugumenterTimeStretch(RawSignalsAugumenter):
    ...
    def __init__(self, stretch_min=0.8, stretch_max=1.25, n_repeats:int = 2, append_original=True, n_jobs=None) -> None:
        super().__init__()
    ...
    def _sig_augument(self, signal:RawSignal,sample_rate):
        sig_list = []
        transformer = TimeStretch(p=1.0, leave_length_unchanged=True, min_rate=self.stretch_min, max_rate=self.stretch_max)
